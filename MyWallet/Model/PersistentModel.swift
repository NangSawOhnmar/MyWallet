...
class PersistentModel {
    ...
    func importInitialData() {
        do {
            ...
            if result.count == 0 {
                ...
                for item in MWCategoryData.list {
    ...
    private func setupDefaultCurrency() {
        ...
        let systemRegionCode  = NSLocale.current.regionCode
        let systemCurrencyCode = NSLocale.current.currencyCode ?? ""
        ...
        if currencyList.contains(where: { $0.countryCode == systemRegionCode}) {
            code = systemRegionCode!
        }
        
        if currencyList.contains(where: { $0.currencyCode == systemCurrencyCode}) {
            NSLocale.setupDefaultCurrencyCode(code: systemCurrencyCode)
        }
        NSLocale.setupDefaultCurrency(symbol: symbol)
        NSLocale.setupDefaultCurrencyCountryCode(code: code)
    ...
    func addSampleData(quantity: Int = 100) {
        // fetch categories
        let categoryRequest: NSFetchRequest<Categories> = Categories.fetchRequest()
    ...
    func updateGeneralCategoriesIfNeeded() {
        ...
        if let results: [Categories] = try? container.viewContext.fetch(fetchRequest) {
            let allDefaultCats = MWCategoryData.list
