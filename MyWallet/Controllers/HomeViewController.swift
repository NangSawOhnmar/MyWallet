...
class HomeViewController: UIViewController {
    ...
    private var monthYearList = [MWMonth] ()
    ...
    private func provideLineData(type: MWRepresentationType) -> LineChartDataSet {
        var mainColor: UIColor = .black
        var gradientFirstColor: UIColor = .clear
        var gradientSecondColor: UIColor = .black
        if type == .totalIncome {
            mainColor = .myAppColor
            gradientFirstColor = .clear
            gradientSecondColor = .myAppColor
        }
    ...
    func configureSegmentedView() {
        ...
        let segmentioViewRect = CGRect(x: frame.minX, y: 0, width: UIScreen.main.bounds.width, height: 50)
        segmentioView = Segmentio(frame: segmentioViewRect)
        segmentioView?.setup(
            content: segmentioContent(),
            style: .onlyLabel,
            options: HomeViewController.segmentioOptions(segmentioStyle: .imageBeforeLabel)
        )
        segmentioView?.selectedSegmentioIndex = segmentioView?.segmentioItems.count ?? 0 - 1
        currentYear = monthYearList.last!.year ?? 1
        currentMonth = monthYearList.last!.month ?? 1
    ...
    func calculateOveralInfo() {
        ...
        let monthlyTotalIncome = Facade.share.model.getTotalMonth(year: currentYear, month: currentMonth, type: .income)
        ...
        overalInfo.append(("Total Cost", monthlyTotalCost.recordPresenter(for: .cost)))
        overalInfo.append(("Total Income", monthlyTotalIncome.recordPresenter(for: .income)))
        overalInfo.append(("Total", monthlyTotal.recordPresenter(for: .all)))

        if totalBudget > 0 {
            let monthlyTotalSave = totalBudget - monthlyTotalCost
            overalInfo.append(("Total Save", monthlyTotalSave.recordPresenter(for: .all)))
        }

        overalInfo.append((" ", " "))

        overalInfo.append(("Daily Average", dailyAverage.recordPresenter(for: .all)))
        overalInfo.append(("Daily Average Cost", dailyAverageCost.recordPresenter(for: .cost)))
        overalInfo.append(("Daily Average Income", dailyAverageIncome.recordPresenter(for: .income)))

        if Date().year() == currentYear && Date().month() == currentMonth {
            overalInfo.append((" ", " "))

            let monthlyForecast = dailyAverage * Double(numDaysAll)
            overalInfo.append(("Monthly Forecast", monthlyForecast.recordPresenter(for: .all)))

            let monthlyForecastCost = dailyAverageCost * Double(numDaysAll)
            overalInfo.append(("Monthly Forecast Cost", monthlyForecastCost.recordPresenter(for: .cost)))

            let monthlyForecastIncome = dailyAverageIncome * Double(numDaysAll)
            overalInfo.append(("Monthly Forecast Income", monthlyForecastIncome.recordPresenter(for: .income)))
        }
    ...
    func calculateCostInfo() {
        ...
        let catWithCost = Facade.share.model.getMonthlyTotalByCategory(year: currentYear, month: currentMonth, type: .cost)
        for result in catWithCost {
            costInfo.append((label: result.category.name, value: result.amount.recordPresenter(for: .cost)))
    ...
    func calculateIncomeInfo() {
        ...
        let catWithCost = Facade.share.model.getMonthlyTotalByCategory(year: currentYear, month: currentMonth, type: .income)
        for result in catWithCost {
            incomeInfo.append((label: result.category.name, value: result.amount.recordPresenter(for: .income)))
        }
    }
}
...
extension HomeViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    private func updateDataAt(index: Int) {
        guard let currentYear = self.monthYearList[index].year,
              let currentMonth = self.monthYearList[index].month else {
...
extension HomeViewController: IAxisValueFormatter {
    func stringForValue( _ value: Double, axis _: AxisBase?) -> String {
        let yearMonth = monthData[Int(0)].month
        let month = yearMonth.shortTitleWithCurrentYear
        return month
    }
}
